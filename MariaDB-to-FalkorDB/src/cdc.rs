...
use crate::source::{mariadb_opts, LogicalRow};

pub async fn run_cdc_stream(cfg: &Config) -> Result<()> {
    let mariadb_cfg = cfg
        .mariadb
        .as_ref()
        .ok_or_else(|| anyhow!("Missing mariadb config for CDC"))?;
    let cdc_cfg = mariadb_cfg.cdc.as_ref();
    ...
    for table_ref in needed_tables {
        ...
        let (db, table) = if parts.len() == 2 {
            ...
            (mariadb_cfg.database.as_deref().unwrap_or(""), parts[0])
        };
    ...
    tracing::info!("Starting MariaDB CDC stream from {} at {}", filename, pos);
