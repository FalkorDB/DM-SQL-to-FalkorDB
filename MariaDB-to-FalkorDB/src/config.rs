...
pub struct MariaDbConfig {
    ...
    #[serde(default)]
    pub cdc: Option<MariaDbCdcConfig>,
}

#[allow(dead_code)]
#[derive(Debug, Deserialize)]
pub struct MariaDbCdcConfig {
    #[serde(default)]
...
/// Source specification: supports either a local JSON file, a MariaDB table,
/// or a custom SELECT statement.
#[derive(Debug, Deserialize)]
pub struct SourceConfig {
    /// Path to a JSON file containing an array of objects, each representing a row.
    pub file: Option<String>,
    /// Optional table name for MariaDB-based sources.
    pub table: Option<String>,
    /// Optional full SELECT statement for MariaDB-based sources.
...
impl Config {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        if let Some(mariadb_cfg) = cfg.mariadb.as_mut() {
            resolve_env_ref(&mut mariadb_cfg.url, "mariadb.url")?;
            resolve_env_ref(&mut mariadb_cfg.host, "mariadb.host")?;
            resolve_env_ref(&mut mariadb_cfg.user, "mariadb.user")?;
            resolve_env_ref(&mut mariadb_cfg.password, "mariadb.password")?;
            resolve_env_ref(&mut mariadb_cfg.database, "mariadb.database")?;
        }
...
mod tests {
    ...
    fn write_temp_file(contents: &str, ext: &str) -> PathBuf {
        ...
        path.push(format!("mariadb_to_falkordb_config_test.{}", ext));
    ...
    fn config_from_yaml_resolves_env_url_and_password() -> Result<()> {
        ...
        let mariadb = cfg.mariadb.expect("expected mariadb config");
        assert_eq!(
            mariadb.url.as_deref(),
            Some("mysql://root:pass@localhost:3306/analytics")
        );
        assert_eq!(mariadb.password.as_deref(), Some("super-secret"));
        Ok(())
    }
