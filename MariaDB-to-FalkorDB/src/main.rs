...
#[derive(Debug, Parser)]
#[command(name = "mariadb-to-falkordb")]
#[command(about = "Load MariaDB/file data into FalkorDB via UNWIND+MERGE", long_about = None)]
struct Cli {
    ...
    #[arg(long, env = "MARIADB_TO_FALKORDB_METRICS_PORT", default_value_t = 9997)]
    metrics_port: u16,

    /// Introspect source MariaDB schema and print a normalized summary.
    #[arg(long)]
...
async fn main() -> Result<()> {
    ...
    if cli.introspect_schema || cli.generate_template {
        ...
        let result = scaffold::introspect_mariadb_schema(&cfg).await?;
