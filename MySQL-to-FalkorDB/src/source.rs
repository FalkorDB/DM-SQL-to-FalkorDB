...
use crate::config::{CommonMappingFields, Config, Mode, MySqlConfig};
...
/// Fetch all rows for a given mapping, from either a file or MySQL.
pub async fn fetch_rows_for_mapping(
    ...
    let mysql_cfg = cfg.mysql.as_ref().ok_or_else(|| {
        anyhow!(
            "No supported source configured for mapping {} (need `file` or MySQL)",
            common.name
        )
    })?;
    if mysql_cfg
    ...
    {
        return Err(anyhow!(
            "mysql.cdc.enabled=true is not implemented yet; use incremental watermark mode for mapping '{}'",
            common.name
        ));
    }

    let sql = build_sql(common, watermark)?;
    fetch_rows_from_mysql(mysql_cfg, &sql, &common.name).await
}
...
pub async fn fetch_rows_page_for_incremental_table(
    ...
    let mysql_cfg = cfg.mysql.as_ref().ok_or_else(|| {
        anyhow!(
            "No MySQL config provided for paged mapping '{}'",
            common.name
        )
    })?;
    if mysql_cfg
...
pub fn build_sql(common: &CommonMappingFields, watermark: Option<&str>) -> Result<String> {
    ...
    if let Some(table) = &common.source.table {
        ...
        Err(anyhow!(
            "MySQL source for mapping '{}' must specify `source.table`, `source.select`, or `source.file`",
            common.name
        ))
    }
}
...
fn mysql_opts(mysql_cfg: &MySqlConfig) -> Result<Opts> {
    ...
    if let Some(password) = mysql_cfg.password.clone() {
        builder = builder.pass(Some(password));
    }
    if let Some(database) = mysql_cfg.database.clone() {
...
async fn fetch_rows_from_mysql(
    ...
    if let Some(timeout_ms) = mysql_cfg.query_timeout_ms {
    ...
    let rows: Vec<Row> = conn.query(sql).await.with_context(|| {
        format!(
            "Failed to execute MySQL query for mapping '{}' with SQL: {}",
            mapping_name, sql
        )
    })?;
    ...
    pool.disconnect()
        .await
        .with_context(|| "Failed to disconnect MySQL pool")?;
...
fn row_to_logical_row(row: Row) -> Result<LogicalRow> {
    ...
    if column_names.len() != values.len() {
        return Err(anyhow!(
            "MySQL row shape mismatch: {} columns but {} values",
            column_names.len(),
            values.len()
        ));
    }
    ...
    for (name, value) in column_names.into_iter().zip(values.into_iter()) {
        out.insert(name, mysql_value_to_json(value));
    }
...
mod tests {
    ...
    /// Optional MySQL connectivity smoke test.
    ///
    /// Uses environment variable:
    /// - MYSQL_URL
    ///
    /// If MYSQL_URL is not set, the test is a no-op and returns Ok(()).
    #[tokio::test]
    async fn mysql_connectivity_smoke_test() -> Result<()> {
        ...
        let rows = fetch_rows_from_mysql(&cfg, "SELECT 1 AS one", "smoke").await?;
