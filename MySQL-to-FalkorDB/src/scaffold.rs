...
use crate::config::{Config, EdgeDirection, MySqlConfig};
...
pub async fn introspect_mysql_schema(cfg: &Config) -> Result<IntrospectionResult> {
    let mysql_cfg = cfg
        .mysql
        .as_ref()
        .ok_or_else(|| anyhow!("mysql config block is required for schema introspection"))?;
    let opts = mysql_opts(mysql_cfg)?;
    let pool = Pool::new(opts);
    let mut conn = pool
        .get_conn()
        .await
        .with_context(|| "Failed to connect to MySQL for introspection")?;

    let db: Option<String> = conn
        .query_first("SELECT DATABASE()")
        .await
        .with_context(|| "Failed to read current MySQL database")?;
    let database = db.ok_or_else(|| {
        anyhow!("No active database selected. Set mysql.database or include DB in mysql.url")
    })?;
