...
#[derive(Debug, Parser)]
#[command(name = "mysql-to-falkordb")]
#[command(about = "Load MySQL/file data into FalkorDB via UNWIND+MERGE", long_about = None)]
struct Cli {
    ...
    #[arg(long, env = "MYSQL_TO_FALKORDB_METRICS_PORT", default_value_t = 9995)]
    metrics_port: u16,

    /// Introspect source MySQL schema and print a normalized summary.
    #[arg(long)]
...
async fn main() -> Result<()> {
    ...
    if cli.introspect_schema || cli.generate_template {
        ...
        let cfg = Config::from_file(&cli.config)?;
        let result = scaffold::introspect_mysql_schema(&cfg).await?;
