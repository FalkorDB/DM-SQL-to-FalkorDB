...
pub struct ClickHouseConfig {
    /// Full HTTP endpoint URL, e.g. "http://localhost:8123/".
...
/// Source specification: supports either a local JSON file, a ClickHouse table,
/// or a custom SELECT statement.
#[derive(Debug, Deserialize)]
pub struct SourceConfig {
    /// Path to a JSON file containing an array of objects, each representing a row.
    pub file: Option<String>,
    /// Optional table name for ClickHouse-based sources.
    pub table: Option<String>,
    /// Optional full SELECT statement for ClickHouse-based sources.
...
impl Config {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        if let Some(ch_cfg) = cfg.clickhouse.as_mut() {
            resolve_env_ref(&mut ch_cfg.url, "clickhouse.url")?;
            resolve_env_ref(&mut ch_cfg.host, "clickhouse.host")?;
            resolve_env_ref(&mut ch_cfg.user, "clickhouse.user")?;
            resolve_env_ref(&mut ch_cfg.password, "clickhouse.password")?;
            resolve_env_ref(&mut ch_cfg.database, "clickhouse.database")?;
        }
...
mod tests {
    ...
    fn write_temp_file(contents: &str, ext: &str) -> PathBuf {
        ...
        path.push(format!("clickhouse_to_falkordb_config_test.{}", ext));
    ...
    fn config_from_yaml_resolves_env_url_and_password() -> Result<()> {
        let url_var = "CLICKHOUSE_TEST_URL";
        let pw_var = "CLICKHOUSE_TEST_PASSWORD";
        env::set_var(url_var, "http://localhost:8123/");
        ...
        let ch = cfg.clickhouse.expect("expected clickhouse config");
        assert_eq!(ch.url.as_deref(), Some("http://localhost:8123/"));
        assert_eq!(ch.password.as_deref(), Some("super-secret"));
        Ok(())
    }
