...
use crate::config::{CommonMappingFields, Config, EntityMapping, Mode, NodeMappingConfig};
...
mod tests {
    ...
    async fn end_to_end_file_load_into_falkordb() -> Result<()> {
        ...
        let graph = std::env::var("FALKORDB_GRAPH")
            .unwrap_or_else(|_| "clickhouse_to_falkordb_load_test".to_string());

        let tmp_dir = std::env::temp_dir();
        let input_path = tmp_dir.join("clickhouse_to_falkordb_nodes.json");
        std::fs::write(
            &input_path,
            r#"[
                {"id": 1, "name": "Alice"},
        ...
        let cfg = Config {
            ...
            state: Some(StateConfig {
                ...
                file_path: Some(
                    std::env::temp_dir()
                        .join("clickhouse_to_falkordb_state.json")
                        .to_string_lossy()
                        .to_string(),
                ),
            }),
