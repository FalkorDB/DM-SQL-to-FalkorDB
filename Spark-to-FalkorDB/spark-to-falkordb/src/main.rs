...
#[derive(Parser, Debug)]
#[command(name = "spark-to-falkordb")]
#[command(about = "Load data from Apache Spark into FalkorDB using declarative mappings.")]
struct Cli {
    ...
    #[arg(long, env = "SPARK_TO_FALKORDB_METRICS_PORT", default_value_t = 9997)]
    metrics_port: u16,

    /// Introspect source Spark schema and print a normalized summary.
    #[arg(long)]
...
async fn main() -> Result<()> {
    ...
    if cli.introspect_schema || cli.generate_template {
        ...
        let result = scaffold::introspect_spark_schema(&cfg).await?;
