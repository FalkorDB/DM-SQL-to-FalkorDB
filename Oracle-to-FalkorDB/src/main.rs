...
#[derive(Debug, Parser)]
#[command(name = "oracle-to-falkordb")]
#[command(about = "Load Oracle/file data into FalkorDB via UNWIND+MERGE", long_about = None)]
struct Cli {
    ...
    #[arg(long, env = "ORACLE_TO_FALKORDB_METRICS_PORT", default_value_t = 9998)]
    metrics_port: u16,

    /// Introspect source Oracle schema and print a normalized summary.
    #[arg(long)]
...
async fn main() -> Result<()> {
    ...
    if cli.introspect_schema || cli.generate_template {
        ...
        let result = scaffold::introspect_oracle_schema(&cfg).await?;
