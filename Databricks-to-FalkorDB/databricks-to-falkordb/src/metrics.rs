...
async fn handle_metrics(_req: Request<Body>) -> Result<Response<Body>, Infallible> {
    ...
    for (name, stats) in guard.iter() {
        body.push_str(&format!(
            "databricks_to_falkordb_mapping_runs{{mapping=\"{}\"}} {}\n",
            name, stats.runs
        ));
        body.push_str(&format!(
            "databricks_to_falkordb_mapping_failed_runs{{mapping=\"{}\"}} {}\n",
            name, stats.failed_runs
        ));
        body.push_str(&format!(
            "databricks_to_falkordb_mapping_rows_fetched{{mapping=\"{}\"}} {}\n",
            name, stats.rows_fetched
        ));
        body.push_str(&format!(
            "databricks_to_falkordb_mapping_rows_written{{mapping=\"{}\"}} {}\n",
            name, stats.rows_written
        ));
        body.push_str(&format!(
            "databricks_to_falkordb_mapping_rows_deleted{{mapping=\"{}\"}} {}\n",
